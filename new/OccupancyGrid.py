...
class OccupancyGrid:
    ...
    def getDistanceVectorized(
        self, x: np.array, y: np.array, theta: np.array
    ) -> np.array:
    ...
    def __getDistanceNumbaOptimized(
        lt: np.array, x: np.array, y: np.array, theta: np.array
    ) -> np.array:
        """
...
if __name__ == "__main__":
    config = MapConfig(
        "Occupancy grid example 2.png", 128, "Occupancy grid example 2_lookup_table.npy"
    )
