...
class SensorModel:
    ...
    def getProbabilityVectorized(self, obsDistances, expectedDistances) -> np.ndarray:
        return self.__getProbabilityNumbaOptimized(
            obsDistances,
            expectedDistances,
            self.prob_table,
            self.config.inv_squash_factor,
        )

    @staticmethod
    @numba.njit(parallel=True)
    def __getProbabilityNumbaOptimized(
        obsDistances, expectedDistances, prob_table, inv_squash_factor
    ) -> np.ndarray:
        """
        ...
        for i in numba.prange(
            n_observations
        ):  # Parallelized outer loop (over observations)
